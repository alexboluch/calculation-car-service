...
def create_logic(new):
    commission_price = calc_commission(new.cleaned_data['auction_price'])
...
def calc_delivery_price(new):
    delivery_land_price = new.cleaned_data['distance_to_port'] + 100
    delivery_all_price = math.ceil(delivery_land_price + new.cleaned_data['delivery_sea_price']
    + 0.01 * (delivery_land_price + new.cleaned_data['delivery_sea_price']) + 30)
...
def calc_end_price(new, delivery_all_price, customs_clearance_price, commission_price, swift_price, registration_price):
    end_price = math.ceil(delivery_all_price + customs_clearance_price + new.cleaned_data['auction_price']
    + commission_price + swift_price + new.cleaned_data['broker_forwarding_price']
    + new.cleaned_data['certification_price'] + new.cleaned_data['company_services_price'] + registration_price)
    return end_price

def calc_swift(new, commission_price):
    swift_price = math.ceil(0.01 * (new.cleaned_data['auction_price'] + commission_price) + 90)
    return swift_price

def calc_registration(new, commission_price):
    registration_price = math.ceil(0.04 * (new.cleaned_data['auction_price'] + commission_price + 500) + 30)
    return registration_price



def create_loan_year(first_year):
    current_year = datetime.now().year
    year_list = list()
    for i in range(current_year, first_year - 1, -1):
        year_list.append((i, i,))
    return tuple(year_list)

def create_loan_engine(start, finish):
    engine_list = list()
    for i in range(finish, start - 100, -100):
        engine_list.append((i, i,))
    return tuple(engine_list)
